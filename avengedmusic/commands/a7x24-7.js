...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    try {
      ...
      // Check and establish voice connection
      ...
      logger.error(`A7X24-7 command error in guild ${guildId}:`, error);
      await interaction.editReply(`There was an error starting the A7X playlist: ${error.message}. Please try again later.`);
      cleanupQueue(guildId, client);
    }
  }
};
...
async function playSong(guildId, queue, channel, client) {
  ...
  try {
    ...
    queue.playing = true; // Ensure playing state is set
